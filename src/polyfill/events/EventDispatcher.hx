...
#if openfl
import openfl.events.EventDispatcher as OpenFlEventDispatcher;
typedef EventDispatcher = OpenFlEventDispatcher;
#else
import polyfill.events.EventPhase;
import polyfill.events.IEventDispatcher;

@:access(polyfill.events.Event)
	...
	public function addEventListener(type:String, listener:Dynamic->Void, useCapture:Bool = false, priority:Int = 0, useWeakReference:Bool = false):Void {
		...
		if (!__eventMap.exists(type)) {
			...
			for (i in 0...list.length) {
				if (Reflect.compareMethods(list[i].callback, listener))
					return;
			}
