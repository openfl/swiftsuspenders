...
class PropertyInjectionPoint extends InjectionPoint
{
	...
	override public function applyInjection(target:Dynamic, targetType:Class<Dynamic>, injector:Injector):Void
	{
		...
		if (provider == null)
		{
			...
			throw(new InjectorMissingMappingError('Injector is missing a mapping to handle injection into property "' + _propertyName + '" of object "' + targetStr + '" with type "' +Type.getClassName(targetType) +'". Target dependency: "' + _mappingId + '"'));
		}
