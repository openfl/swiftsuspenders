			...
			if (hasInject) {
				...
				if (injectObject != null) {
					for (j in 0...injectObject.length) {
						...
						if (key == "optional")
							optional = value.toLowerCase() == "true";
						if (key == "name")
							injectName = value;
					}
				}
				...
				for (node in _currentFactoryXML.iterator()) {
					if (node.nodeType == Xml.Element) {
						if (node.nodeName == value) {
							...
							var injectionPoint = Type.createInstance(injectionPointType,
								[node.nodeName, parameters, requiredParameters, 0x3FFFFFFF]); // ORDER: isNaN(order) ? Limits.IntMax:order
